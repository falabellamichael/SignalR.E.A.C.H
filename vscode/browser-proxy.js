...
function handle(req, res) {
  ...
  if (req.headers.host !== new URL(origin).host || (req.method !== 'GET' && req.method !== 'HEAD')
      || (req.headers.origin && req.headers.origin !== origin)) {
    respond(res, 403, { 'Content-Type': 'text/plain' }, 'forbidden');
    return;
  }
  let sameOriginAsset = false;
  try { sameOriginAsset = new URL(req.headers.referer).origin === origin; } catch (_) { /* no page referrer */ }
  const raw = req.url || '/';
  ...
  } else if (raw.startsWith('/') && !raw.startsWith('/r/') && targets.has(token) && sameOriginAsset) {
