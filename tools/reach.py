...
def register_autostart():
    ...
    if ngrok:
        ...
        if gh:
            lines.append("\"%s\" tools\\reach.py publish --quiet" % sys.executable)
    else:
    ...
    if result.returncode != 0:
        raise SystemExit("error: schtasks failed: %s"
                         % (result.stderr or result.stdout or "").strip()[:300])
    print("  autostart registered: SimpleREACH (logon task) -> " + str(BAT_PATH))
