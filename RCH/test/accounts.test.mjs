...
test('plan grants and redeemed usage are durable, idempotent and separated',t=>{
 const folder=mkdtempSync(join(tmpdir(),'reach-accounts-'));let store;
 t.after(()=>{store?.close();rmSync(folder,{recursive:true,force:true});});
 const path=join(folder,'accounts.sqlite');store=new AccountStore(path,{models:[model]});
 ...
 store=new AccountStore(path,{models:[model]});store.creditRedemption(intent.redemptionId,'event1');
