...
const { AgentLoop } = require('../agent/agent-loop.cjs');
{
  ...
  server.listen(0, '127.0.0.1', async () => {
    ...
    try {
      ...
      assert.ok(!finalAgent.messages[2].content.includes('reach 0.1'), 'tool provenance should not duplicate the file contents');
      assert.ok(finalAgent.messages[3].content.includes('reach 0.1'), 'tool summary should contain the file contents');
