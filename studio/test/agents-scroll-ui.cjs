...
(async () => {
  ...
  for (const [width, height, zoom, drawer] of [[1440, 900, 1, false], [1000, 640, 1, true], [1420, 980, 1.25, true]]) {
    ...
    assert.equal(await run(`document.querySelector('#agent-activity .activity-details').open`), false, 'Activity starts collapsed');
    ...
    await run(`chatScroll.scrollTop = 0; document.querySelector('#agent-activity .activity-details').open = true; document.querySelector('#agent-activity .activity-result').open = true; document.querySelector('#agent-activity .activity-steps').scrollTop = 0;`);
    await delay(100);
    ...
    assert.equal(await run(`(() => {
      const scrollers = [...chatScroll.querySelectorAll('*')].filter(el =>
        !el.classList.contains('activity-steps') && el.getBoundingClientRect().height > 0 && el.scrollHeight > el.clientHeight + 2 && /auto|scroll/.test(getComputedStyle(el).overflowY));
      return scrollers.map(el => el.className).join(',');
    })()`), '', 'Only the activity step list should have a nested vertical scroll area');
    assert.equal(await run(`(() => { const steps=document.querySelector('#agent-activity .activity-steps'); return steps.clientHeight <= Math.min(320,innerHeight*.35)+1 && steps.clientHeight >= 150 && steps.scrollHeight > steps.clientHeight; })()`), true, 'Expanded activity shows a useful bounded viewport');
    await wheelOver('#agent-activity .activity-steps');
    assert.ok(await run(`document.querySelector('#agent-activity .activity-steps').scrollTop > 20`), 'Wheel over activity scrolls its steps');
    assert.equal(await run('chatScroll.scrollTop'), 0, 'Inner activity scrolling leaves the conversation in place');
    await run(`document.querySelector('#agent-activity .activity-steps').scrollTop = 0; document.querySelector('#agent-activity .activity-steps').focus()`);
    win.webContents.sendInputEvent({type:'keyDown',keyCode:'PageDown'});
    win.webContents.sendInputEvent({type:'keyUp',keyCode:'PageDown'});
    await delay(250);
    assert.ok(await run(`document.querySelector('#agent-activity .activity-steps').scrollTop > 20`), 'Activity supports keyboard scrolling');
    await run(`(() => { const steps=document.querySelector('#agent-activity .activity-steps'); steps.scrollTop=steps.scrollHeight; })()`);
    const pageAtEnd = await run('chatScroll.scrollTop');
    await wheelOver('#agent-activity .activity-steps');
    assert.ok(await run('chatScroll.scrollTop') > pageAtEnd + 20, 'At the end of activity, wheel scrolling continues through the conversation');
  ...
  await run(`document.querySelector('#agent-activity .activity-details').open=false; chatScroll.scrollTop=200`);
  await until(`document.querySelector('#agent-activity').classList.contains('is-concealed')`, 'Activity did not hide on downward scroll');
  await run(`chatScroll.scrollTop=140`);
  await until(`!document.querySelector('#agent-activity').classList.contains('is-concealed')`, 'Activity did not return on upward scroll');
  const summaryPoint = await run(`(() => { const r=document.querySelector('#agent-activity .activity-details > summary').getBoundingClientRect(); return {x:Math.round(r.left+Math.min(r.width/2,100)),y:Math.round(r.top+r.height/2)}; })()`);
  win.focus(); win.webContents.focus();
  win.webContents.sendInputEvent({type:'mouseMove',...summaryPoint});
  win.webContents.sendInputEvent({type:'mouseDown',...summaryPoint,button:'left',clickCount:1});
  win.webContents.sendInputEvent({type:'mouseUp',...summaryPoint,button:'left',clickCount:1});
  await until(`document.querySelector('#agent-activity .activity-details').open`, 'Activity summary did not open');
  await delay(100);
  const openedActivity = await run(`({hidden:document.querySelector('#agent-activity').classList.contains('is-concealed'),top:chatScroll.scrollTop,focused:document.activeElement?.className})`);
  assert.equal(openedActivity.hidden, false, `Opening activity must keep it visible: ${JSON.stringify(openedActivity)}`);
  assert.ok(Math.abs(openedActivity.top - 140) <= 2, 'Opening activity must not jump the conversation');
  await run(`chatScroll.scrollTop+=60`);
  await until(`document.querySelector('#agent-activity').classList.contains('is-concealed')`, 'Expanded activity did not hide on a later downward scroll');
  await run(`chatScroll.scrollTop-=60`);
  await until(`!document.querySelector('#agent-activity').classList.contains('is-concealed')`, 'Expanded activity did not return on a later upward scroll');
  await run(`document.querySelector('#agent-activity .activity-details').open=false; chatScroll.scrollTop=0`);
  ...
  await run(`(() => {
    const child=subCard('scroll-child','Child worker','fixture',1);
    ReachActivity.team({teamRunId:'scroll-team',type:'subagent',agentId:'scroll-child',netType:'agent-started'},child);
    handleTeamEvent({teamRunId:'scroll-team',type:'member-start',index:1});
  })()`);
  await delay(200);
  assert.equal(await run(`scrollTeam.wrap.querySelectorAll('.activity-details').length >= 3 && [...scrollTeam.wrap.querySelectorAll('.activity-details')].every(el=>!el.open)`), true, 'Team members and spawned workers all start collapsed');
  await run(`ReachActivity.ingest({agentId:currentAgent.id,type:'run-state',status:'running'});`);
  await delay(200);
  assert.equal(await run(`document.querySelector('#agent-activity .activity-details').open`), false, 'A new running trace starts collapsed');
  await run(`document.querySelector('#agent-activity .activity-details').open=true; ReachActivity.ingest({agentId:currentAgent.id,type:'request-start'});`);
  await delay(200);
  assert.equal(await run(`document.querySelector('#agent-activity .activity-details').open`), true, 'Live updates preserve manual expansion');
  await run(`ReachActivity.ingest({agentId:currentAgent.id,type:'run-state',status:'completed'});`);
  await delay(200);
  assert.equal(await run(`document.querySelector('#agent-activity .activity-details').open`), true, 'Completion preserves manual expansion');
  await run(`ReachActivity.ingest({agentId:currentAgent.id,type:'run-state',status:'running'});`);
  await delay(200);
