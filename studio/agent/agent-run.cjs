...
/* Reach Studio — agent run control, ported from the VS Code extension's
 * media/agent-run.js. The protocol text the model sees, the strict fence
 * parser, and the decide() state machine are identical so behaviour matches.
 */
...
function decide(run, input) {
  ...
  const open = state.todos.filter(t => !['completed', 'cancelled'].includes(t.status));
