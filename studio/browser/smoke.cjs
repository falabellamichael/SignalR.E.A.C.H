...
module.exports = async function browserSmoke(win, browser, outputDir) {
  ...
  try {
    ...
    const draftBeforeDismiss = await ui('return composerInput.value;');
    ...
    await new Promise(resolve => setTimeout(resolve, 100));
    assert.equal(await first.webContents.executeJavaScript("!!document.querySelector('[data-reach-browser-highlight]')"), true);
    const away = await first.webContents.executeJavaScript("(() => { const r = document.querySelector('p').getBoundingClientRect(); return {x:r.x+5,y:r.y+5}; })()");
    leftClick(away);
    await until(() => ui("return document.querySelector('#browser-selection').classList.contains('hidden');"));
    assert.equal(await first.webContents.executeJavaScript("!!document.querySelector('[data-reach-browser-highlight]')"), false);
    await browser.selectElement(browser.tabs.get(firstId), point);
    await until(() => ui("return !document.querySelector('#browser-selection').classList.contains('hidden');"));
    await ui("composerInput.dispatchEvent(new PointerEvent('pointerdown', { button: 0, bubbles: true }));");
    await until(() => first.webContents.executeJavaScript("!document.querySelector('[data-reach-browser-highlight]')"));
    assert.equal(await ui('return composerInput.value;'), draftBeforeDismiss);
    assert.equal(await ui("return document.querySelector('#browser-selection').classList.contains('hidden');"), true);
    console.log('SELECTION DISMISS SMOKE OK: selected element retained, away/outside left clicks clear, chat draft preserved.');
