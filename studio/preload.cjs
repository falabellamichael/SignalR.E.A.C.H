...
contextBridge.exposeInMainWorld('reach', {
  ...
  agents: {
    ...
    send: (id, text, attachmentIds = []) => ipcRenderer.invoke('agents:send', { id, text, attachmentIds }),
  ...
  teams: {
    ...
    run: (teamId, task, dir, agentId, useHistory = true) => ipcRenderer.invoke('teams:run', { teamId, task, dir, agentId, useHistory }),
